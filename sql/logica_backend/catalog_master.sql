...
-- ║  plan de ejecución.                                     ║
-- ╚══════════════════════════════════════════════════════════╝
DROP FUNCTION IF EXISTS fn_cat_get_productos();
...
-- ╔══════════════════════════════════════════════════════════╗
-- ║  FUNCIÓN 4: fn_cat_delete_producto                      ║
-- ╠══════════════════════════════════════════════════════════╣
-- ║  Propósito  : Eliminar un producto protegiendo la       ║
-- ║               integridad referencial del negocio.       ║
-- ║  Llamada PHP: SELECT fn_cat_delete_producto(53)         ║
-- ║  Retorna    : JSON {ok: bool, msg: '...'}               ║
-- ║                                                         ║
-- ║  FLUJO:                                                 ║
-- ║  1. Admin pulsa "Eliminar" en la fila del producto.     ║
-- ║  2. Frontend muestra confirmación → envía DELETE/POST.  ║
-- ║  3. PHP ejecuta SELECT fn_cat_delete_producto(id).      ║
-- ║  4. La función evalúa 2 barreras de integridad.         ║
-- ║  5. Si libre → DELETE de tab_Productos.                 ║
-- ║  6. Si bloqueada → retorna error con motivo.            ║
-- ║  7. PHP reenvía el {ok, msg} al frontend.               ║
-- ║                                                         ║
-- ║  BARRERAS DE SEGURIDAD:                                 ║
-- ║  1. Integridad Contable: Si el reloj se vendió, no se   ║
-- ║     puede borrar (violación de históricos fiscales).    ║
-- ║  2. Integridad de Transacción: Si está en un carrito,   ║
-- ║     el borrado afectaría el checkout de un cliente.     ║
-- ║                                                         ║
-- ║  RECOMENDACIÓN:                                         ║
...
BEGIN
    ...
    -- Buscamos si el producto existe en tab_Detalle_Orden (líneas de factura).
    -- El límite (LIMIT 1) optimiza la búsqueda al detenerse en el primer hallazgo.
    ...
    -- Evita que el producto desaparezca mientras clientes lo tienen listo para comprar.
    ...
    -- EJECUCIÓN: Tras superar las barreras, se procede con la eliminación física.
    DELETE FROM tab_Productos WHERE id_producto = p_id;

    RETURN json_build_object('ok', true,
        'msg', 'El registro ha sido removido permanentemente del catálogo.');
END;
$$ LANGUAGE plpgsql;
...
-- ╚══════════════════════════════════════════════════════════╝
DROP FUNCTION IF EXISTS fn_cat_get_marcas();
...
-- ╔══════════════════════════════════════════════════════════╗
-- ║  FUNCIÓN 8: fn_cat_delete_marca                         ║
-- ╠══════════════════════════════════════════════════════════╣
-- ║  Propósito  : Eliminar físicamente una marca si y solo si║
-- ║               no tiene productos vinculados.            ║
-- ║  Llamada PHP: SELECT fn_cat_delete_marca(10)            ║
-- ║  Retorna    : JSON {ok: bool, msg: text}                ║
-- ║                                                         ║
-- ║  FLUJO:                                                 ║
-- ║  1. Admin pulsa "Eliminar" en la fila de la marca.      ║
-- ║  2. PHP ejecuta SELECT fn_cat_delete_marca(id).         ║
-- ║  3. La función cuenta productos asociados.              ║
-- ║  4. Si dependencias > 0 → retorna error con cantidad.   ║
-- ║  5. Si limpio → DELETE de tab_Marcas irreversible.      ║
-- ║  6. Retorna {ok, msg} → PHP reenvía al frontend.        ║
-- ║                                                         ║
-- ║  PROTECCIÓN:                                            ║
-- ║  Evita "huérfanos" en tab_Productos. Si hay dependencias,║
...
BEGIN
    -- AUDITORÍA DE INTEGRIDAD: Validamos la tabla de productos.
    SELECT COUNT(id_producto) INTO v_count 
    FROM tab_Productos 
    WHERE id_marca = p_id;

    -- BLOQUEO: Si el conteo es positivo, abortamos para proteger el catálogo.
    IF v_count > 0 THEN
        RETURN json_build_object('ok', false,
            'msg', 'Restricción de Integridad: Existen ' || v_count || 
            ' productos asociados a esta marca. Operación abortada para evitar datos huérfanos.');
    END IF;

    -- EJECUCIÓN: Eliminación física irreversible.
    DELETE FROM tab_Marcas WHERE id_marca = p_id;

    RETURN json_build_object('ok', true, 'msg', 'La marca ha sido borrada permanentemente.');
END;
$$ LANGUAGE plpgsql;
...
-- ╚══════════════════════════════════════════════════════════╝
DROP FUNCTION IF EXISTS fn_cat_get_categorias();
...
-- ╔══════════════════════════════════════════════════════════╗
-- ║  FUNCIÓN 12: fn_cat_delete_categoria                    ║
-- ╠══════════════════════════════════════════════════════════╣
-- ║  Propósito  : Eliminar una categoría principal asegurando ║
-- ║               que no existan dependencias de segundo     ║
-- ║               nivel (Subcategorías) ni productos.       ║
-- ║  Llamada PHP: SELECT fn_cat_delete_categoria(3)          ║
-- ║  Retorna    : JSON {ok: bool, msg: text}                ║
-- ║                                                         ║
-- ║  FLUJO:                                                 ║
-- ║  1. Admin solicita eliminación de categoría.            ║
-- ║  2. PHP ejecuta SELECT fn_cat_delete_categoria(id).     ║
-- ║  3. La función evalúa 2 barreras de integridad.         ║
-- ║  4. Barrera 1: ¿Existen subcategorías hijas?             ║
-- ║  5. Barrera 2: ¿Existen productos asociados?             ║
-- ║  6. Si libre → DELETE de tab_Categorias.                ║
-- ║  7. Retorna {ok, msg} → PHP reenvía al frontend.        ║
-- ║                                                         ║
-- ║  MURALLA DE PROTECCIÓN DOBLE:                           ║
-- ║  1. Dependencias Jerárquicas (tab_Subcategorias):        ║
-- ║     Si existen "hijas", el borrado se cancela.          ║
-- ║  2. Dependencias de Negocio (tab_Productos):             ║
-- ║     Si hay relojes asociados, el borrado se cancela.    ║
-- ╚══════════════════════════════════════════════════════════╝
CREATE OR REPLACE FUNCTION fn_cat_delete_categoria(
    p_id INTEGER  -- ID de la categoría a purgar
)
RETURNS JSON AS $$
BEGIN
    -- PROTECCIÓN 1: Integridad de la Taxonomía.
    -- Buscamos subcategorías que dependan de este ID padre.
    IF EXISTS (SELECT 1 FROM tab_Subcategorias WHERE id_categoria = p_id LIMIT 1) THEN
        RETURN json_build_object('ok', false,
            'msg', 'Acción Denegada: La categoría contiene subcategorías vinculadas.');
    END IF;

    -- PROTECCIÓN 2: Integridad del Stock.
    -- Buscamos productos que referencien directamente esta categoría.
    IF EXISTS (SELECT 1 FROM tab_Productos WHERE id_categoria = p_id LIMIT 1) THEN
        RETURN json_build_object('ok', false,
            'msg', 'Acción Denegada: Hay productos en el catálogo que dependen de esta categoría.');
    END IF;

    -- PURGA: Si ambas condiciones son negativas, se permite la eliminación.
    DELETE FROM tab_Categorias WHERE id_categoria = p_id;

    RETURN json_build_object('ok', true, 'msg', 'Categoría eliminada de forma permanente del sistema.');
END;
$$ LANGUAGE plpgsql;
...
-- ╚══════════════════════════════════════════════════════════╝
DROP FUNCTION IF EXISTS fn_cat_get_subcategorias();
...
-- ╔══════════════════════════════════════════════════════════╗
-- ║  FUNCIÓN 16: fn_cat_delete_subcategoria                 ║
-- ╠══════════════════════════════════════════════════════════╣
-- ║  Propósito  : Remover una subcategoría siempre que no    ║
-- ║               existan productos asociados a ella.       ║
-- ║  Llamada PHP: SELECT fn_cat_delete_subcategoria(1, 4)    ║
-- ║  Retorna    : JSON {ok: bool, msg: text}                ║
-- ║                                                         ║
-- ║  FLUJO:                                                 ║
-- ║  1. Admin solicita eliminación de subcategoría.         ║
-- ║  2. PHP ejecuta fn_cat_delete_subcategoria(cat, sub).   ║
-- ║  3. La función cuenta productos con esa clasificación.  ║
-- ║  4. Si dependencias > 0 → retorna error con cantidad.   ║
-- ║  5. Si limpio → DELETE por clave compuesta.             ║
-- ║  6. Retorna {ok, msg} → PHP reenvía al frontend.        ║
-- ║                                                         ║
-- ║  PROTECCIÓN DE INTEGRIDAD:                              ║
-- ║  Busca en tab_Productos usando el filtro de categoría + ║
-- ║  subcategoría. Si hay coincidencias, aborta la operación.║
-- ╚══════════════════════════════════════════════════════════╝
...
BEGIN
    -- AUDITORÍA: Verificamos si hay productos que dependen de esta clasificación.
    SELECT COUNT(id_producto) INTO v_count
    FROM tab_Productos
    WHERE id_categoria = p_id_cat AND id_subcategoria = p_id_sub;

    -- BARRERA: Si hay dependencias, informamos al admin para evitar inconsistencias.
    IF v_count > 0 THEN
        RETURN json_build_object('ok', false,
            'msg', 'Restricción de Integridad: Existen ' || v_count || 
            ' productos vinculados a esta subcategoría específica.');
    END IF;

    -- PURGA: Eliminación física irreversible.
    DELETE FROM tab_Subcategorias 
    WHERE id_categoria = p_id_cat AND id_subcategoria = p_id_sub;

    RETURN json_build_object('ok', true, 'msg', 'Subcategoría removida del árbol con éxito.');
END;
$$ LANGUAGE plpgsql;
...
-- ╚══════════════════════════════════════════════════════════╝
DROP FUNCTION IF EXISTS fn_cat_dropdown_marcas();
...
-- ╚══════════════════════════════════════════════════════════╝
DROP FUNCTION IF EXISTS fn_cat_dropdown_categorias();
...
-- ╚══════════════════════════════════════════════════════════╝
DROP FUNCTION IF EXISTS fn_cat_dropdown_subcategorias(integer);
...
-- ╔══════════════════════════════════════════════════════════╗
-- ║  FUNCIÓN 23: fn_cat_delete_servicio                     ║
-- ╠══════════════════════════════════════════════════════════╣
-- ║  Propósito  : Eliminar un servicio blindando la agenda    ║
-- ║               del taller.                               ║
-- ║  Llamada PHP: SELECT fn_cat_delete_servicio(5)          ║
-- ║  Retorna    : JSON {ok: bool, msg: text}                ║
-- ║                                                         ║
-- ║  FLUJO:                                                 ║
-- ║  1. Admin solicita eliminación de un servicio.          ║
-- ║  2. PHP ejecuta SELECT fn_cat_delete_servicio(id).      ║
-- ║  3. La función cuenta citas vinculadas en tab_Reservas. ║
-- ║  4. Si citas > 0 → retorna error con cantidad.          ║
-- ║  5. Si limpio → DELETE del portafolio.                  ║
-- ║  6. Retorna {ok, msg} → PHP reenvía al frontend.        ║
-- ║                                                         ║
-- ║  PROTECCIÓN DE INTEGRIDAD:                              ║
-- ║  Busca en tab_Reservas (agenda de clientes). Si hay      ║
-- ║  citas activas o históricas, se bloquea el borrado para ║
-- ║  preservar la trazabilidad del taller.                  ║
-- ╚══════════════════════════════════════════════════════════╝
CREATE OR REPLACE FUNCTION fn_cat_delete_servicio(
    p_id BIGINT -- ID del servicio a remover
)
RETURNS JSON AS $$
DECLARE
    v_count BIGINT; -- Contador de citas encontradas
BEGIN
    -- AUDITORÍA: Verificación en la tabla de Reservas.
    SELECT COUNT(id_reserva) INTO v_count 
    FROM tab_Reservas 
    WHERE id_servicio = p_id;

    -- BARRERA: Protección de la agenda histórica y futura.
    IF v_count > 0 THEN
        RETURN json_build_object('ok', false,
            'msg', 'Restricción de Integridad: Este servicio posee ' || v_count || 
            ' citas registradas en agenda. Use la desactivación lógica en su lugar.');
    END IF;

    -- PURGA: Eliminación física si no hay rastros en agenda.
    DELETE FROM tab_Servicios WHERE id_servicio = p_id;

    RETURN json_build_object('ok', true, 'msg', 'Servicio removido permanentemente del portafolio.');
END;
$$ LANGUAGE plpgsql;
