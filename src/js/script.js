...
function updateHeaderUser(user) {
    ...
    newBtn.addEventListener('click', (e) => {
        ...
        if (user && user.rol === 'admin') {
            window.location.href = `${API_CONFIG.appUrl}/src/admin/admin.html`;
        } else {
            window.location.href = `${API_CONFIG.appUrl}/src/user/user.html`;
        }
    });
}
        ...
        loginForm.addEventListener('submit', async function (e) {
            ...
            try {
                ...
                if (data.ok) {
                    ...
                    setTimeout(() => {
                        if (data.redirect) {
                            // Si el backend devuelve "src/admin/admin.html", y estamos en el root,
                            // queremos ir a "appUrl/src/admin/admin.html".
                            // Pero espera, mi fix en el backend devolvió "src/...".
                            // Si appUrl es "http://localhost", queda "http://localhost/src/admin/admin.html". Correcto.
                            // Si appUrl es "http://localhost/rd_watch", queda "http://localhost/rd_watch/src/admin/admin.html". Correcto.

                            // PERO, debemos asegurarnos que data.redirect no empiece con slash si appUrl no termina con slash
                            // API_CONFIG.appUrl NO termina en slash (confirmado en config.js)

                            // OJO: Si el backend ya devuelve 'src/...', está bien.
                            // Lo unimos con la URL base de la app.
                            window.location.href = `${API_CONFIG.appUrl}/${data.redirect}`;
                        }
                    }, 1000);
                } else {
