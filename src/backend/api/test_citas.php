...
try {
    if ($method === 'GET') {
        ...
        if ($rol === 'admin') {
            ...
            $stmt = $pdo->prepare("SELECT fn_citas_list_cliente(?)");
    ...
    elseif ($method === 'POST') {
        ...
        if (isset($data['action']) && $data['action'] === 'update_status') {
            ...
            // Llamada opaca
            $stmt = $pdo->prepare("SELECT fn_citas_update_status(?, ?, ?)");
        ...
        $stmt = $pdo->prepare("SELECT fn_citas_create(?, ?, ?::date, ?, ?)");
    ...
    http_response_code(500);
    echo json_encode(['ok' => false, 'msg' => 'Error en servicio de citas: ' . $e->getMessage()]);
}