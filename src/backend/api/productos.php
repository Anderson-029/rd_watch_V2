...
try {
    switch ($method) {
        ...
        //   Si pasa ambas → elimina físicamente
        case 'DELETE':
            requireRole('admin');
            validateCsrfToken(null, true);
            ...
            $stmt = $pdo->prepare("SELECT fn_cat_delete_producto(?)");
            $stmt->execute([$pid]);
            echo json_encode(json_decode($stmt->fetchColumn(), true));
            break;
    ...
    http_response_code(500);
    echo json_encode(['ok' => false, 'msg' => 'Error crítico en el servidor (Productos): ' . $e->getMessage()]);
}